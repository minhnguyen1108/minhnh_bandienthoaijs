function HideReview(n, t) {
    confirm("Bạn có muốn ẩn đánh giá này!") && $.post("/api/review/hide", { id: n }, function(n) {
        if (n.Errors) $.toast({ heading: "Có lỗi xảy ra.", text: n.Title, showHideTransition: "fade", icon: "error", hideAfter: 15e3 });
        else {
            $.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 5e3 });
            var r = $(t).parent().find("input[name=commentReq]").val(),
                i = JSON.parse(r);
            console.log(i);
            $.post(i.path, i, function(n) { $(t).parent().html(n) })
        }
    })
}

function RemoveReview(n, t) {
    confirm("Bạn có muốn xóa đánh giá này!") && $.post("/api/review/remove", { id: n }, function(n) {
        if (n.Errors) $.toast({ heading: "Có lỗi xảy ra.", text: n.Title, showHideTransition: "fade", icon: "error", hideAfter: 15e3 });
        else {
            $.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 5e3 });
            var r = $(t).parent().find("input[name=commentReq]").val(),
...
function HideComment(n, t) {
    confirm("Bạn có muốn ẩn bình luận này!") && $.post("/api/comment/hide", { id: n }, function(n) {
        if (n.Errors) $.toast({ heading: "Có lỗi xảy ra.", text: n.Title, showHideTransition: "fade", icon: "error", hideAfter: 15e3 });
        else {
            $.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 5e3 });
            var r = $(t).parent().find("input[name=commentReq]").val(),
...
function RemoveComment(n, t) {
    confirm("Bạn có muốn xóa bình luận này!") && $.post("/api/comment/remove", { id: n }, function(n) {
        if (n.Errors) $.toast({ heading: "Có lỗi xảy ra.", text: n.Title, showHideTransition: "fade", icon: "error", hideAfter: 15e3 });
        else {
            $.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 5e3 });
            var r = $(t).parent().find("input[name=commentReq]").val(),
...
function UserLocation(n) { $.cookie("_city", n, { path: "/" }) }

function scrollTopFunction() { document.body.scrollTop > 20 || document.documentElement.scrollTop > 20 ? $("#backtoTop").show() : $("#backtoTop").hide() }
...
function init_Review() {
    ...
    i = { max_value: 5, step_size: .5, selected_symbol_type: "hhicon", cursor: "default", readonly: !1, change_once: !1 };
    $(".rating").rate(i);
    $(".rating").on("change", function(i, r) {
        var u = validateForm(t);
        u == "" ? ($(t).find("input[name=Rate]").val(r.to), n = $(t).serializeObject(), $.post("/api/review", n, function(i) { i.Errors ? $.toast({ heading: "Có lỗi khi gửi review.", text: i.Message, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }) : ($.toast({ heading: i.Title, showHideTransition: "fade", icon: "success", hideAfter: 15e3 }), $(t).find("textarea").val(""), ajaxLoadReview(n.ModelID, n.SystemTypeID, 1, $("#reviewContent"))) })) : $.toast({ heading: "Bạn cần kiểm tra lại thông tin.", text: u, showHideTransition: "fade", icon: "error", hideAfter: 15e3 })
    })
}
...
function ajaxLoadReview(n, t, i, r) {
    var u = { path: "/ajax/review", modelId: n, systemTypeId: t, p: 1 };
    $.post(u.path, u, function(n) { $(r).html(n) })
}

function ajaxLoadComment(n, t, i, r, u, f) {
    var e = { path: "/ajax/comment", modelId: n, systemTypeId: t, parent: u, child: f, p: 1 };
    $.post(e.path, e, function(n) { $(r).html(n) })
}

function displayRate(n) { $(n).rate({ max_value: 5, step_size: .5, selected_symbol_type: "hhicon", cursor: "default", readonly: !0 }) }
...
function init_cityChange(n) {
    $("#SystemCityID").change(function() {
        ...
        $.get(r, function(n) { $("#SystemDistrictID").html(n) })
    });
    $("#SystemCityID").change()
}

function validatePhone(n) { return /((09|03|07|08|05)+([0-9]{8})\b)/g.test(n) }

function validateEmail(n) { return /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(n) == !1 ? !1 : !0 }

function updateViewProduct(n) { $.post("/api/product/updateview/" + n) }

function updateViewRepair(n) { $.post("/api/repair/updateview/" + n) }
...
function submitSearch(n) { var t = $(n).find("#kwd").val(); if (t == "") return $.toast({ heading: "Chú ý.!", text: "Vui lòng nhập từ khóa tìm kiếm.", icon: "error" }), !1 }

function submitSubscription(n) { var t = $(n).find("#email").val(); return validateEmail(t) ? $.post("/Ajax/AddSubScription", $(n).serialize(), function(t) { t.Errors ? $.toast({ heading: "Lỗi", text: t.Message, icon: "error" }) : ($.toast({ heading: "Đăng ký thành công.!", text: t.Message, icon: "success" }), $(n).find("#email").val("")) }) : $.toast({ heading: "Lỗi", text: "Email không hợp lệ, vui lòng kiểm tra lại.", icon: "error" }), !1 }

function compareAutocomplete(n) { $("#kwdCompare").autocomplete({ serviceUrl: "/api/search?template=" + n.template + "&ptype=" + n.ptype + "&ignore=" + n.ignore, deferRequestBy: 200, maxHeight: 350, formatResult: autoComplateFormat }) }

function autoComplateFormat(n) { var t = '<div class="search-item" onclick="location.href=\'' + n.data.uid + "'\">"; return t += '<div class="img"><img src="' + n.data.image + '" /><\/a><\/div>', t += '   <div class="info">', t += '       <h2><a href="' + n.data.uid + '">' + n.data.title + "<\/a><\/h2>", t += "       <h3><strike>" + n.data.lastprice + "<\/strike> " + n.data.price + "<\/h3>", t += "   <\/div>", t += "<\/div>", t + "<\/div>" }
...
function init_quickOrder() {
    $("a.changeOption").click(function() {
        ...
        $.post(t, postValue, function(n) { $("#popup-modal").html(n).modal({ escapeClose: !1, clickClose: !1, closeClass: "icon-minutes", closeText: " " }) })
    });
    $("input.cart-promote:not(:checked)").click(function() {
        var n = [];
        $("input.cart-promote:checked").each(function() { n.push($(this).val()) });
        var t = n.join(","),
            i = $("#quickForm input[name=SKU]").val(),
            r = $("#quickForm input[name=COLOR]").val(),
            u = "/ajax/quickorder?skuId=" + i + "&colorId=" + r + "&offerId=" + t;
        postValue = $("#quickForm").serializeObject();
        $.post(u, postValue, function(n) { $("#popup-modal").html(n).modal({ escapeClose: !1, clickClose: !1, closeClass: "icon-minutes", closeText: " " }) })
    });
    ...
    var n = Math.max.apply(null, $(".options .option a").map(function() { return $(this).width() }).get());
    $(".options .option a").width(n)
}

function wishProduct(n, t) {
    var i = '<div class="cart-msg">';
    t ? $.post("/api/account/wish", { ID: n }, function(n) {
        ...
        i += "<\/div>";
        $.toast({ text: i, position: "bottom-center", stack: !1, loader: !1, hideAfter: 5e3 })
    }) : (i += '<p><i class="icon-minutes text-red"><\/i> <span>Bạn cần đăng nhập để sử dụng chức năng này<\/span><\/p>', i += '<a class="button button-red" href="/account/login">Đăng nhập/Đăng ký<\/a>', i += "<\/div>", $.toast({ text: i, position: "bottom-center", stack: !1, loader: !1, hideAfter: 5e3 }))
}
...
function checkWarranty(n) {
    ...
    return i == "" ? (postValue = $(t).serializeObject(), $.post("/api/warranty", postValue, function(n) { n.ok ? ($.toast({ text: "Tìm thấy thông tin bảo hành.", showHideTransition: "fade", hideAfter: 5e3 }), $(".container .title").text(n.title), $(".container .imei").text(n.imei), $(".container .buyDate").text(n.buyDate), $(".container .expiryDate").text(n.expiryDate), $(".container .store").text(n.store), $(".container .found").show(), $(".container .not-found").hide()) : ($.toast({ text: "Không tìm thấy thông tin bảo hành.", showHideTransition: "fade", hideAfter: 5e3 }), $(".container .imei").text(postValue.IMEI), $(".container .found").hide(), $(".container .not-found").show()) })) : $.toast({ heading: "Bạn cần nhập mã IEMI của sản phẩm.", text: i, showHideTransition: "fade", icon: "error", hideAfter: 5e3 }), !1
}

function validFormCheckOrder(n) {
    var t = $(n),
        i = validateForm(t);
    return i == "" ? (postValue = $(t).serializeObject(), $.post("/api/order/check", postValue, function(n) { n.Errors ? $.toast({ text: n.Title, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }) : ($.toast({ text: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 5e3 }), setTimeout(function() { location.href = n.Message }, 500)) })) : $.toast({ heading: "Bạn cần nhập mã đơn hàng.", text: i, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }), !1
}

function validFormQuickSub(n) {
    var t = $(n),
        i = validateForm(t);
    return i == "" ? (postValue = $(t).serializeObject(), $.post("/api/cart/quicksub", postValue, function(n) { n.Errors ? $.toast({ heading: "Có lỗi khi gửi thông tin đăng ký.", text: n.Message, showHideTransition: "fade", icon: "error", hideAfter: 1e4 }) : ($.toast({ heading: n.Title, text: n.Message, showHideTransition: "fade", icon: "success", position: "bottom-center", hideAfter: 1e4 }), $("#popup-modal a.close-modal").click()) })) : $.toast({ heading: "Bạn cần kiểm tra lại thông tin.", text: i, showHideTransition: "fade", icon: "error", hideAfter: 1e4 }), !1
}
...
function validFormQuickOrder(n) {
    var i = $(n),
        r = validateForm(i),
        t;
    return r == "" ? (t = [], $("#quickFormPromote").find(":checked").each(function() { t.push($(this).val()) }), $("#quickForm #Promote").val(t.join(",")), postValue = $(i).serializeObject(), $.post("/api/cart/quickorder", postValue, function(n) { n.Errors ? $.toast({ heading: "Có lỗi khi gửi thông tin đơn hàng.", text: n.Message, showHideTransition: "fade", icon: "error", hideAfter: 1e4 }) : ($.toast({ heading: n.Title, text: n.Message, showHideTransition: "fade", icon: "success", position: "bottom-center", hideAfter: 1e4 }), $("#popup-modal a.close-modal").click()) })) : $.toast({ heading: "Bạn cần kiểm tra lại thông tin.", text: r, showHideTransition: "fade", icon: "error", hideAfter: 1e4 }), !1
}

function validFormOrder(n) {
    var t = $(n),
        i = validateForm(t);
    return i == "" ? (postValue = $(t).serializeObject(), $.post("/api/cart/checkout", postValue, function(n) { n.Errors ? $.toast({ heading: "Có lỗi khi gửi thông tin đơn hàng.", text: n.Message, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }) : ($.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 15e3 }), location.href = "/cart/checkout") })) : $.toast({ heading: "Bạn cần kiểm tra lại thông tin.", text: i, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }), !1
}
...
function cartChangeOffer(n) {
    var t = cartGetCookie(),
        ...
        i;
    r >= 0 && (i = [], $("#of_" + n).find(":checked").each(function() { i.push($(this).val()) }), t[r].offer = i);
    $.cookie("_cart", JSON.stringify(t), { path: "/" });
    console.log(t);
    cartReload()
}
...
function cartMinutes(n) {
    ...
    if (i >= 0) {
        if (t[i].number == 1) { cartDelete(n); return }
        t[i].number--;
        $.cookie("_cart", JSON.stringify(t), { path: "/" })
    }
    cartReload()
...
function cartPlus(n) {
    var t = cartGetCookie(),
        i = t.findIndex(t => t.sku == n);
    i >= 0 && (t[i].number++, $.cookie("_cart", JSON.stringify(t), { path: "/" }));
    cartReload()
}

function cartChange() { cartReload() }

function cartDelete(n) {
    var t = cartGetCookie(),
        i = t.findIndex(t => t.sku == n);
    i >= 0 && (t.splice(i, 1), $.cookie("_cart", JSON.stringify(t), { path: "/" }));
    cartReload()
}

function cartEdit() { cartReload() }

function cartReload() {
    var t = cartGetCookie(),
        n;
    t.length > 0 ? (n = "/Ajax/CartItems", $.get(n, function(n) { $("#cartInfo").html(n) })) : location.href = "/gio-hang"
}

function btnInstallment() { $(".btnInstallment").click(function() { var n = $(".btnInstallment").hasClass("disabled") || $(".btnQuickOrder").hasClass("btn-outstock"); if (n) return !1 }) }
        ...
        if (!s) {
            ...
            u.forEach(function(n) {
                if (t) {
                    ...
                    i >= 0 ? t[i].number++ : (t.push({ sku: n, number: 1, color: e ? e : r }), flyToElement(o, $("#cart-total")))
                } else t.push({ sku: n, number: 1, color: r })
            });
            $.cookie("_cart", JSON.stringify(t), { path: "/" });
            checkTotalCart();
            n.preventDefault();
            ...
            i += "<\/div>";
            $.toast({ text: i, position: "bottom-center", stack: !1, loader: !1, hideAfter: 1e4 })
        }
    });
    checkTotalCart()
}
...
function checkTotalCart() {
    var t = 0,
        n = cartGetCookie();
    n && n.length > 0 && (t = n.reduce(function(n, t) { return n + t.number }, 0));
    $("#cart-total").text(t)
}
...
function replyComment(n) {
    ...
    return i == "" ? (postValue = $(t).serializeObject(), $.post("/api/comment", postValue, function(n) {
        if (n.Errors) $.toast({ heading: "Có lỗi khi gửi bình luận.", text: n.Message, showHideTransition: "fade", icon: "error", hideAfter: 15e3 });
        else {
            $.toast({ heading: n.Title, showHideTransition: "fade", icon: "success", hideAfter: 15e3 });
            $(t).find("textarea").val("");
    ...
    })) : $.toast({ heading: "Bạn cần kiểm tra lại thông tin.", text: i, showHideTransition: "fade", icon: "error", hideAfter: 15e3 }), !1
}

function showSticker(n) {
    var t = $.cookie("sticker_" + n),
        i;
    (t == "undefined" || t == null) && (i = "/Ajax/AdsStickerFooter/" + n, $.post(i, function(n) { $("#sticker-modal").html(n) }))
}

function showPopup(n) {
    var t = $.cookie("popup_" + n),
        i;
    (t == "undefined" || t == null) && (i = "/Ajax/AdsPopup/" + n, $.post(i, function(n) { $("#popup-modal").html(n).modal({ closeClass: "icon-minutes", closeText: " " }) }))
}

function scrollTo(n) { $("html, body").animate({ scrollTop: $(n).offset().top - 50 }, 500) }
...
function flyToElement(n, t) {
    var e = $(this),
        ...
        i = $(n).clone(),
        u, f;
    $(i).css({ position: "absolute", top: $(n).offset().top + "px", left: $(n).offset().left + "px", opacity: 1, "z-index": 1e3 });
    $("body").append($(i));
    u = $(t).offset().left + $(t).width() / 2 - $(n).width() / r / 2;
    f = $(t).offset().top + $(t).height() / 2 - $(n).height() / r / 2;
    $(i).animate({ opacity: .4, left: u, top: f, width: $(n).width() / r, height: $(n).height() / r }, 500, function() { $(t).fadeOut("fast", function() { $(t).fadeIn("fast", function() { $(i).fadeOut("fast", function() { $(i).remove() }) }) }) })
}

function hh_home() {
    $(".lr-slider").owlCarousel({ items: 5, nav: !0, itemClass: "lr-item", loop: !1, autoplay: !0, autoplayHoverPause: !0 });
    window.jssor_1_slider_init = function() {
        ...
        var i = { $class: $JssorSlideshowRunner$, $AutoPlay: 1, $FillMode: 2, $SlideDuration: 500, $Idle: 1e4, $ArrowNavigatorOptions: { $Class: $JssorArrowNavigator$ }, $ThumbnailNavigatorOptions: { $Loop: !1, $Class: $JssorThumbnailNavigator$, $ChanceToShow: 4, $ActionMode: 1, $SpacingX: 0, $ParkingPosition: 0, $NoDrag: !0, $Align: 2 } },
...
function marketFilters(n) {
    ...
    setTimeout(function() { $("#marketFilter .list").removeClass("hide") }, 50);
...
function quickOrder() {
    $(".btnQuickOrder").click(function(n) {
        ...
        if (!u) {
            ...
            n.preventDefault();
            this.blur();
            $.get(r, function(n) { $("#popup-modal").html(n).modal({ escapeClose: !1, clickClose: !1, closeClass: "icon-minutes", closeText: " " }) })
        }
    })
}
...
function quickSub() {
    $(".btnQuickSubscription").click(function(n) {
        var t = $(this).attr("data-id"),
            i = "/ajax/quickSub?productId=" + t;
        n.preventDefault();
        this.blur();
        $.get(i, function(n) { $("#popup-modal").html(n).modal({ escapeClose: !1, clickClose: !1, closeClass: "icon-minutes", closeText: " " }) })
    })
}
...
function(n) {
    n.isScrollToFixed = function(t) { return !!n(t).data("ScrollToFixed") };
    n.ScrollToFixed = function(t, i) {
        ...
        function nt() { var n = u.options.limit; return n ? typeof n == "function" ? n.apply(r) : n : 0 }

        function l() { return s === "fixed" }

        function tt() { return s === "absolute" }

        function e() { return !(l() || tt()) }

        function b() {
            if (!l()) {
                var n = r[0].getBoundingClientRect();
                h.css({ display: r.css("display"), width: n.width, height: n.height, float: r.css("float") });
                cssOptions = { "z-index": u.options.zIndex, position: "fixed", top: u.options.bottom == -1 ? y() : "", bottom: u.options.bottom == -1 ? "" : u.options.bottom, "margin-left": "0px" };
        ...
        function it() {
            ...
            cssOptions = { position: "absolute", top: n, left: t, "margin-left": "0px", bottom: "" };
            u.options.dontSetWidth || (cssOptions.width = r.css("width"));
            r.css(cssOptions);
            s = "absolute"
        }

        function f() { e() || (v = -1, h.css("display", "none"), r.css({ "z-index": ut, width: "", position: d, left: "", top: rt, "margin-left": "" }), r.removeClass("scroll-to-fixed-fixed"), u.options.className && r.removeClass(u.options.className), s = null) }

        function k(n) { n != v && (r.css("left", c - n), v = n) }

        function y() { var n = u.options.marginTop; return n ? typeof n == "function" ? n.apply(r) : n : 0 }
        ...
        function st() { return u.options.bottom ? u.options.bottom : 0 }
        ...
        var p = !1,
            ...
            v = -1,
            h = null,
            g = function() { r.is(":visible") ? (p = !1, a()) : f() },
            et = function() { window.requestAnimationFrame ? requestAnimationFrame(a) : a() },
            lt = function() {
                var n = document.body,
                    t, i, r, u, f;
        ...
        u.init = function() {
            ...
            r.bind("resize.ScrollToFixed", function() { h.height(r.height()) });
    ...
    n.ScrollToFixed.defaultOptions = { marginTop: 0, limit: 0, bottom: -1, zIndex: 1e3, baseClassName: "scroll-to-fixed-fixed" };
    n.fn.scrollToFixed = function(t) { return this.each(function() { new n.ScrollToFixed(this, t) }) }
}(jQuery);
! function(n, t, i, r) {
    function u(t, i) {
        this.settings = null;
...
! function(n, t, i, r, u, f, e) {
    function b(n) { return function(t) { return s.$Round(t * n) / n } }

    function nt(n, t, i) {
        function e(n) { return n * s.u / 180 }

        function v(n, t, i, r, u, f, e, o, s, h, c, l, a, v, y, p, w, b, k, d, g, nt, tt, it, rt, ut, ft, et, ot, st, ht, ct) { return [n * w + t * g + i * rt + r * ot, n * b + t * nt + i * ut + r * st, n * k + t * tt + i * ft + r * ht, n * d + t * it + i * et + r * ct, u * w + f * g + e * rt + o * ot, u * b + f * nt + e * ut + o * st, u * k + f * tt + e * ft + o * ht, u * d + f * it + e * et + o * ct, s * w + h * g + c * rt + l * ot, s * b + h * nt + c * ut + l * st, s * k + h * tt + c * ft + l * ht, s * d + h * it + c * et + l * ct, a * w + v * g + y * rt + p * ot, a * b + v * nt + y * ut + p * st, a * k + v * tt + y * ft + p * ht, a * d + v * it + y * et + p * ct] }

        function o(n, t) { return v.apply(r, (t || u).concat(n)) }
        var f = this,
        ...
        f.Sg = function() { return "matrix3d(" + u.join(",") + ")" };
        f.Og = function() { return "matrix(" + [u[0], u[1], u[4], u[5], u[12], u[13]].join(",") + ")" }
    }

    function y() {
        function s(n, t) { i.push({ ec: n, gc: t }) }

        function h(n, t) { o.f(i, function(r, u) { r.ec == n && r.gc === t && i.splice(u, 1) }) }

        function c() { i = [] }

        function e() {
            o.f(r, function(n) { o.$RemoveEvent(n.Ne, n.ec, n.gc, n.Xe) });
            r = []
        }
        var t = this,
            f, i = [],
            r = [];
        t.Oc = function() { return f };
        t.a = function(n, t, i, u) {
            o.$AddEvent(n, t, i, u);
            r.push({ Ne: n, ec: t, gc: i, Xe: u })
        };
        t.T = function(n, t, i, u) { o.f(r, function(f, e) { f.Ne === n && f.ec == t && f.gc === i && f.Xe == u && (o.$RemoveEvent(n, t, i, u), r.splice(e, 1)) }) };
        t.Ve = e;
        t.$On = t.addEventListener = s;
        t.$Off = t.removeEventListener = h;
        t.k = function(t) {
            var r = [].slice.call(arguments, 1);
            o.f(i, function(i) { i.ec == t && i.gc.apply(n, r) })
        };
        t.$Destroy = function() { f || (f = u, e(), c()) }
    }
    ...
    var s, h, o, l, d, g, c;
    new function() {};
    s = { u: i.PI, l: i.max, j: i.min, H: i.ceil, G: i.floor, P: i.abs, gb: i.sin, Cb: i.cos, Oe: i.tan, Xf: i.atan, Yf: i.atan2, xb: i.sqrt, B: i.pow, dd: i.random, $Round: i.round, Y: function(n, t) { var i = s.B(10, t || 0); return s.$Round(n * i) / i } };
    h = n.$Jease$ = {
        $Swing: function(n) { return -s.Cb(n * s.u) / 2 + .5 },
        $Linear: function(n) { return n },
        $InQuad: function(n) { return n * n },
        $OutQuad: function(n) { return -n * (n - 2) },
        $InOutQuad: function(n) { return (n *= 2) < 1 ? 1 / 2 * n * n : -1 / 2 * (--n * (n - 2) - 1) },
        $InCubic: function(n) { return n * n * n },
        $OutCubic: function(n) { return (n -= 1) * n * n + 1 },
        $InOutCubic: function(n) { return (n *= 2) < 1 ? 1 / 2 * n * n * n : 1 / 2 * ((n -= 2) * n * n + 2) },
        $InQuart: function(n) { return n * n * n * n },
        $OutQuart: function(n) { return -((n -= 1) * n * n * n - 1) },
        $InOutQuart: function(n) { return (n *= 2) < 1 ? 1 / 2 * n * n * n * n : -1 / 2 * ((n -= 2) * n * n * n - 2) },
        $InQuint: function(n) { return n * n * n * n * n },
        $OutQuint: function(n) { return (n -= 1) * n * n * n * n + 1 },
        $InOutQuint: function(n) { return (n *= 2) < 1 ? 1 / 2 * n * n * n * n * n : 1 / 2 * ((n -= 2) * n * n * n * n + 2) },
        $InSine: function(n) { return 1 - s.Cb(s.u / 2 * n) },
        $OutSine: function(n) { return s.gb(s.u / 2 * n) },
        $InOutSine: function(n) { return -1 / 2 * (s.Cb(s.u * n) - 1) },
        $InExpo: function(n) { return n == 0 ? 0 : s.B(2, 10 * (n - 1)) },
        $OutExpo: function(n) { return n == 1 ? 1 : -s.B(2, -10 * n) + 1 },
        $InOutExpo: function(n) { return n == 0 || n == 1 ? n : (n *= 2) < 1 ? 1 / 2 * s.B(2, 10 * (n - 1)) : 1 / 2 * (-s.B(2, -10 * --n) + 2) },
        $InCirc: function(n) { return -(s.xb(1 - n * n) - 1) },
        $OutCirc: function(n) { return s.xb(1 - (n -= 1) * n) },
        $InOutCirc: function(n) { return (n *= 2) < 1 ? -1 / 2 * (s.xb(1 - n * n) - 1) : 1 / 2 * (s.xb(1 - (n -= 2) * n) + 1) },
        $InElastic: function(n) { if (!n || n == 1) return n; return -(s.B(2, 10 * (n -= 1)) * s.gb((n - .075) * 2 * s.u / .3)) },
        $OutElastic: function(n) { if (!n || n == 1) return n; return s.B(2, -10 * n) * s.gb((n - .075) * 2 * s.u / .3) + 1 },
        ...
        $InBack: function(n) { var t = 1.70158; return n * n * ((t + 1) * n - t) },
        $OutBack: function(n) { var t = 1.70158; return (n -= 1) * n * ((t + 1) * n + t) + 1 },
        $InOutBack: function(n) { var t = 1.70158; return (n *= 2) < 1 ? 1 / 2 * n * n * (((t *= 1.525) + 1) * n - t) : 1 / 2 * ((n -= 2) * n * (((t *= 1.525) + 1) * n + t) + 2) },
        $InBounce: function(n) { return 1 - h.$OutBounce(1 - n) },
        $OutBounce: function(n) { return n < 1 / 2.75 ? 7.5625 * n * n : n < 2 / 2.75 ? 7.5625 * (n -= 1.5 / 2.75) * n + .75 : n < 2.5 / 2.75 ? 7.5625 * (n -= 2.25 / 2.75) * n + .9375 : 7.5625 * (n -= 2.625 / 2.75) * n + .984375 },
        $InOutBounce: function(n) { return n < 1 / 2 ? h.$InBounce(n * 2) * .5 : h.$OutBounce(n * 2 - 1) * .5 + .5 },
        $GoBack: function() { return 1 - s.P(1) },
        $InWave: function(n) { return 1 - s.Cb(n * s.u * 2) },
        $OutWave: function(n) { return s.gb(n * s.u * 2) },
        $OutJump: function(n) { return 1 - ((n *= 2) < 1 ? (n = 1 - n) * n * n : (n -= 1) * n * n) },
        $InJump: function(n) { return (n *= 2) < 1 ? n * n * n : (n = 2 - n) * n * n },
        $Early: s.H,
        $Late: s.G,
        $Mid: s.$Round,
        $Mid2: b(2),
    ...
    o = n.$Jssor$ = new function() {
        function tt(n, t, i) { return n.indexOf(t, i) }

        function er() {
            if (!yt) {
                yt = { td: "ontouchstart" in n || "createTouch" in t };
                var i;
        ...
        function or() { return wt(vt) }

        function sr() { return wt(oi) }

        function hr() { return wt(si) }

        function st() { return wt(), hi > 537 || b > 42 || w == vt && b >= 11 }
        ...
        function cr(n) {
            ...
            return function(i) {
                if (!t) {
                    ...
                    t = c(["", "WebKit", "ms", "Moz", "O", "webkit"], function(t) { var u = t + (t ? r : n); return i.style[u] != e && u }) || n
                }
                return t
            }
        }
        ...
        function lr(t) { return n.SVGElement && t instanceof n.SVGElement }

        function ar(n) { return {}.toString.call(n) }
        ...
        function kt(n) { return n == r ? String(n) : ai[ar(n)] || "object" }

        function gt(n) { for (var t in n) return u }

        function rt(n) { try { return kt(n) == "object" && !n.nodeType && n != n.window && (!n.constructor || {}.hasOwnProperty.call(n.constructor.prototype, "isPrototypeOf")) } catch (t) {} }

        function vr(n, t) { return { x: n, y: t } }

        function yr(n, t) { setTimeout(n, t || 0) }

        function p(n, t) { return n === e ? t : n }

        function ni(n) { n.constructor === ni.caller && n.F && n.F.apply(n, ni.caller.arguments) }

        function it(t) { return t || n.event }

        function ut(t, i, u) {
            if (u !== e) t.style[i] = u == e ? "" : u;
            else { var f = t.currentStyle || t.style; return u = f[i], u == "" && n.getComputedStyle && (f = t.ownerDocument.defaultView.getComputedStyle(t, r), f && (u = f.getPropertyValue(i) || f[i])), u }
        }
        ...
        function l(n, t, i, u) {
            ...
            return t & 2 && (f = "px"), t & 4 && (f = "%"), t & 16 && (f = "em"), e = t & 8 && cr(n), t &= -9, u = u || (t ? pr : ut),
                function(t, o) { i && o && (o *= i); var s = u(t, e ? e(t) : n, o, f); return i && s != r ? s / i : s }
        }

        function ft(n) { return function(t, i) { d(t, n, i) } }
        ...
        function yi(n) { return "rect(" + n.y + "px " + n.t + "px " + n.m + "px " + n.x + "px)" }

        function pi(n, t, i, u) {
            for (u = u || "u", n = n ? n.firstChild : r; n; n = n.nextSibling)
                if (n.nodeType == 1) { if (ct(n, u) == t) return n; if (!i) { var f = pi(n, t, i, u); if (f) return f } }
        }

        function wi(n, t, i, u) { var f, e; for (u = u || "u", f = [], n = n ? n.firstChild : r; n; n = n.nextSibling) n.nodeType == 1 && (ct(n, u) == t && f.push(n), i || (e = wi(n, t, i, u), a(e) && (f = f.concat(e)))); return f }
        ...
        function bi(n) { return t.createElement(n) }
        ...
        function ct(n, t) { return d(n, t) || d(n, "data-" + t) }

        function lt(n, t) { return d(n, "class", t) || "" }

        function ii(n) { var t = {}; return c(n, function(n) { n != e && (t[n] = n) }), t }

        function ri(n, t) { return n && n.match(t || rr) || r }

        function ui(n, t) { return ii(ri(n || "", t)) }
        ...
        function br(n, t, i) { lt(n, fi(" ", g(ti(ui(lt(n)), ui(t)), ui(i)))) }

        function ki() { c([].slice.call(arguments, 0), function(n) { i.Md(n) ? ki.apply(r, n) : n && n.$Destroy && n.$Destroy() }) }

        function ei(n, t, r) { var u = n.cloneNode(!t); return r || i.Oh(u, "id"), u }
        ...
        function di(n, t, i, r, u, f, e) { var h, o, c, v, l, a; return u && (h = u[0], o = u[1], e && (c = s.l(o * 2, 1), v = c * (e - 1) + 1, i = (i * v - h) / c, i > 0 && (o /= c, h = 0, l = s.H(i) - 1, i = i - l, i > o && l < e - 1 && (i = 1 - i))), i = (i - h) / o, i = s.j(s.l(i, 0), 1)), f && (i = i * f, a = s.G(i), i - a && (i -= a), i = s.j(s.l(i, 0), 1)), (u || f) && (i = s.Y(i, 3)), n + t * r(i) }

        function dr(n, t, r, u) {
            function l(t) { f[t] = n[t] }
            ...
            var s, f, h, o;
            return n = n || {}, s = {}, f = {}, h = { cc: 0, Wb: 0, sX: 1, sY: 1, r: 0, rX: 0, rY: 0, tX: 0, tY: 0, tZ: 0, kX: 0, kY: 0 }, o = {}, (!st() || t) && (o.tZ = i.Kc, o.tX = i.Kc, o.tY = i.Kc), (st() || t) && (o.x = a, o.y = v), c(n, function(n, t) {
                (o[t] || l)(t)
            }), c(f, function(n, t) { h[t] != e && (s[t] = n, delete f[t]) }), gt(s) && (f.Kd = s), f
        }
        ...
        function gi(n, t) {
            ...
            return e && r.reverse(), h = [], c(r, function(n, t) { h[n] = t }), h
        }

        function gr(n, t, i, r) { for (var o, f = [], h = i ? s.H((n + r) / 2) : n, e = 1 / (t * (h - 1) + 1), u = 0; u < n; u++) o = i ? s.G((u + r) / 2) : u, f[u] = [o * t * e, e]; return f }
        ...
        function nu(n, t, r) {
            ...
            function ut(n, t) { for (var u = 0, f = 0, i = 0, r = t ? 6 : 0; r < a(n); r += 6) u += n[r], f += n[r + 1], i++; return { x: i ? u / i : e, y: i ? f / i : e } }

            function u(n) {
                function r(t) { return { x: n[t], y: n[t + 1] } }
                ...
                return { lb: n, qb: (a(n) - 2) / 6, kc: f.x, ic: f.y, Nc: l, Vc: e, vc: c }
            }

            function w(n) {
                function h(n) { return n.replace(/[\^\s]*([mhvlzcsqta]|\-?\d*\.?\d+)[,\$\s]*/gi, " $1").replace(/([mhvlzcsqta])/gi, " $1").trim().split("  ").map(l) }

                function l(n) { return n.split(" ").map(v) }

                function v(n, t) { return t === 0 ? n : +n }
            ...
            function o(n, t) { return s.Y(n, 2) + "," + s.Y(t, 2) }

            function et(n) { for (var i = "M" + o(n[0], n[1]), t = 2; t < a(n) - 2; t += 6) i += "C", i += o(n[t], n[t + 1]) + " ", i += o(n[t + 2], n[t + 3]) + " ", i += o(n[t + 4], n[t + 5]); return i }

            function ot(n) { var t = ""; return c(n, function(n) { t += et(n) }), t }

            function st(n) { for (var i = [], t = 0; t < a(n) - 2; t += 6) i.push(u(n.slice(t, t + 8))); return i }
            ...
            function b(n, t) {
                function v(i) { return t.Nc[i] - n.Nc[i] }

                function o(n, t) { return t = t || 0, s.Y((n - t + s.u * 101) % (s.u * 2) - s.u + t, 8) }
                ...
                p = gr(r, it);
                w = gi(r, h.o);
                t.ad = function(n, i) { if (n >= 0 && n <= t.qb) return di(0, 1, i, nt, p[w[n % r]]) }
            }

            function k(n, t, i, r, f) {
                function w(n) { for (var r, t = [0, .2, 0, .09, .09, 0, .2, 0, .31, 0, .4, .09, .4, .2, .4, .31, .31, .4, .2, .4, .09, .4, 0, .31, 0, .2], i = 0; i < a(t); i += 2) t[i] += n.kc - .2, t[i + 1] += n.ic - .2; return r = u(t), r.vc = n.vc, r }
                var e, o, nt, h, v;
                if (n = i[f] = n || w(t), t = r[f] = t || w(n), e = n.qb, o = t && t.qb, e < o) return k(t, n, r, i, f);
                if (o < e) {
                    for (var d = st(t.lb), g = e / o, y = e - o, l = 0, p = 0; p < 10 && l < y; p++) nt = g + g * p / 10, h = 0, c(d, function(n, t) { var r, i; return h += n.qb, r = s.$Round((t + 1) * nt), h < r && (i = s.j(r - h, y - l), n.qb += i, l += i, h += i), y < l });
                    v = [];
                    c(d, function(n) {
            ...
            function g(n, t) {
                ...
                c(n, function(i, r) { k(i, t[r], n, t, r) })
            }
            ...
            function y(n, t, i, r, u, f) {
                ...
                if (u >= 0 && u < a(n.lb)) {
                    ...
                    return f[u] = o, f[u + 1] = h, { x: o, y: h }
                }
            }
            var l, v;
            n = n || { d: r.$Elmt ? d(r.$Elmt, "d") : "" };
            var nt = r.$Easing,
                h = r.Bd || {},
                f = h.r,
                tt = f == 0,
            ...
            return l = w(n.d), v = w(t.d), g(l, v), {
                    ...
                    return c(l, function(n, t) {
                        for (var o = [], u = v[t], f = 0; f < a(n.lb); f += 6) {
                            var e = u.ad(f / 6, i),
                                h = p(u.ad(f / 6 - 1, i), e),
                                c = p(u.ad(f / 6 + 1, i), e),
                                l = { x: n.kc + (u.kc - n.kc) * e, y: n.ic + (u.ic - n.ic) * e },
                                s = y(n, u, l, e, f, o);
                            y(n, u, s, (e + h) / 2, f - 2, o);
                            y(n, u, s, (e + c) / 2, f + 2, o)
                        }
                ...
                X: function(n) { return n && n.d || "" },
                sb: ft("d")
            }
        }

        function tu(n) { return g({ wc: i.Ph(n) ? n : h.$Linear }, n) }

        function et(n, t, f, v, y) {
            function ri(n) { var t = kt[n] || {}; return o.v({}, f, { $Easing: lt[n] || lt.wc || r, Pc: ni[n] || r, Bd: t, $During: dt[n] || r, $Round: gt[n] || r, Mf: t.rd, Yc: i.Nd(tt) ? tt : tt && tt[n], le: 0 }) }

            function it(n) { return a(n) % 2 ? n.concat(n) : n }

            function w(n, t, i) { return n = v ? dr(n, wt, i && f.Lf, i && f.Of) : n || {}, y ? g({}, t, n) : n }

            function b(n, t) {
                function i(n) { return n == "transparent" ? r : n || t }

                function u(n, t) { return n = i(n), t = i(t), !n && t && (n = t.slice(0), n[3] = 0), n || [0, 0, 0, 0] }
                return function(t, i, r) {
                    i = u(i, t);
                    t = u(t, i);
                    var f = et(t, i, o.v({ Yc: [0, 0, 0, 4] }, r));
                    return { E: function(n) { return f.E(n) }, X: function(n) { return "rgba(" + n.join(",") + ")" }, sb: n }
                }
            }
            ...
            function ui(n, t, i) {
                ...
                var r = i.$Easing || h.$Linear,
                    ...
                    o = t - n,
                    c = p(i.Yc, 2);
                return { E: function(t) { return s.Y(di(n, o, t, r, u, f, e), c) }, X: ht, sb: i.Pc }
            }

            function vt(n, t, i, r) { return { E: et(n, t, i).E, X: function(n) { return n.join(",") }, sb: r } }

            function fi(n, t, i) { return vt(it(n || [0]), it(t), i, ft("stdDeviation")) }

            function ei(n, t, i) {
                function o(n) { for (var t = n; a(t) < u * f / e;) t = t.concat(n); return t }
                var f = a(t),
                    u, e, r;
                for (t = it(t), n || (n = t.slice(0), c(n, function(t, i) { i % 2 && (n[i] = 0) })), n = it(n), u = a(n), r = 1; r <= u && r <= f; r++) u % r || f % r || (e = r);
                return vt(o(n), o(t), i, ft("stroke-dasharray"))
            }

            function oi(n, t, i) { return { E: et(n, t, i).E, X: function(n) { return (n.y || n.x || n.m - i.$OriginalHeight || n.t - i.$OriginalWidth) && yi(n) || "" }, sb: l("clip") } }
            ...
            function si(n, t, i) {
                ...
                var r = i.Nf,
                    u;
                return i.Ib ? (ct = ft("transform"), u = function(n) { return f(n).Og() }) : (ct = li, u = ti ? function(n) { return f(n).Sg() } : wr), { E: et(n, t, i).E, sb: ct, X: u || ht }
            }
            ...
            t = w(t, e, u);
            var ti = st(),
                ii = y ? { c: oi, Kd: si, pt: nu, bl: fi, da: ei, fc: b(ft("fill"), [0, 0, 0, 1]), sc: b(ft("stroke")), cl: b(l("color"), [0, 0, 0, 1]), bgc: b(l("backgroundColor")), bdc: b(l("borderColor")), rp: hi } : {},
                tt = f.Yc || y && { o: 4, so: 4, Kd: 4, ls: 4, lh: 4, sX: 4, sY: 4 };
            return c(t, function(t, i) {
                ...
                pt[t] = r;
                n.E = function(n) { return r[s.$Round(n * d)] };
                n.X = ht
            }), ut.E = function(t) { var i = g(u, at ? [] : {}, n); return o.f(k, function(n, r) { i[r] = n.E(t) }), i }, ut
        }

        function at(n, t) { c(t, function(t, i) { ot[i] && ot[i](n, t) }) }

        function ir(n) { var t; return o.Nd(n) ? t = tr[n] : o.Sh(n) && (t = {}, c(n, function(n, i) { t[i] = ir(n) })), t || n }
        ...
        var i = this,
            ...
            k = parseFloat,
            li = l("transform", 8),
            bt = Array.isArray || function(n) { return kt(n) == "array" },
            ai = {},
            vi, dt, ot, nr, tr;
        c(["Boolean", "Number", "String", "Function", "Array", "Date", "RegExp", "Object"], function(n) { ai["[object " + n + "]"] = n.toLowerCase() });
        i.ud = er;
        i.ue = or;
        i.He = sr;
        i.gg = st;
        i.Cd = function() { return b };
        i.$Delay = yr;
        i.wc = p;
        i.W = function(n, t) { return t.call(n), g({}, n) };
        i.F = ni;
        i.$GetElement = function(n) { return i.ag(n) && (n = t.getElementById(n)), n };
        i.xc = function(n) {
            var t = [];
        ...
        i.Zf = it;
        i.$EvtSrc = function(n) { n = it(n); var r = n.target || n.srcElement || t; return r.nodeType == 3 && (r = i.Lb(r)), r };
        i.Fe = function(n) { return n = it(n), n.relatedTarget || n.toElement };
        i.Je = function(n) { return n = it(n), n.which || [0, 1, 3, 0, 2][n.button] || n.charCode || n.keyCode };
        i.Tc = function(n) { return n = it(n), { x: n.clientX || 0, y: n.clientY || 0 } };
        i.Ue = function(n, t) { return vr(n.x - t.x, n.y - t.y) };
        i.eg = function(n, t) { return n.x >= t.x && n.x <= t.t && n.y >= t.y && n.y <= t.m };
        i.Ie = function(n, t) {
            var r = o.dg(t),
                u = o.Tc(n);
            return i.eg(u, r)
        };
        i.Ib = function(n) { return lr(i.Lb(n)) };
        vi = { r: ["rotate"], sX: ["scaleX", 2], sY: ["scaleY", 2], tX: ["translateX", 1], tY: ["translateY", 1], kX: ["skewX"], kY: ["skewY"] };
        ...
        i.we = function(n, t) { li(n, t == 1 ? "" : "scale(" + t + ")") };
        ...
        i.Z = function(n, t) { var i = [].slice.call(arguments, 2); return function() { var r = i.concat([].slice.call(arguments, 0)); return t.apply(n, r) } };
        i.bd = function(n, r) {
        ...
        i.dg = function(n) { var t = n.getBoundingClientRect(); return { x: t.left, y: t.top, w: t.right - t.left, h: t.bottom - t.top, t: t.right, m: t.bottom } };
        i.jb = function(n, t) { for (var r = [], i = n.firstChild; i; i = i.nextSibling)(t || i.nodeType == 1) && r.push(i); return r };
        i.$FindChild = pi;
        i.rg = function(n, t) { return n.getElementsByTagName(t) };
        i.kb = function(n, i, r, u) {
            var e, f;
            r = r || "u";
            do {
                if (n.nodeType == 1 && (r && (f = ct(n, r)), f && f == p(i, f) || u == n.tagName)) { e = n; break }
                n = o.Lb(n)
            } while (n && n != t.body);
            return e
        };
        i.qe = function(n) { return ii(["INPUT", "TEXTAREA", "SELECT"])[n.tagName] };
        i.v = g;
        i.Zg = ti;
        i.Pd = function(n, t) {
            if (n) {
        ...
        i.Ph = function(n) { return kt(n) == "function" };
        i.Md = bt;
        i.ag = function(n) { return kt(n) == "string" };
        i.Nd = function(n) { return !bt(n) && !isNaN(k(n)) && isFinite(n) };
        i.f = c;
        i.Th = gt;
        i.Sh = rt;
        i.bc = function() { return bi("DIV") };
        i.Rd = function() { return bi("SPAN") };
        i.Kc = function() {};
        i.g = d;
        i.hb = ct;
        i.q = function(n, t, r) { var u = i.od(ct(n, t)); return isNaN(u) && (u = r), u };
        i.Hc = function(n, t, i) { return ri(d(n, t), i) };
        i.Fd = ii;
        i.Ud = ri;
        i.mg = function(n) { return n && (n = n.toLowerCase()), n };
        i.je = fi;
        i.Lb = function(n) { return n.parentNode };
        i.Fc = function(n) { i.nb(n, "none") };
        i.mb = function(n, t) { i.nb(n, p(t, u) ? "" : "none") };
        i.Oh = function(n, t) { n.removeAttribute(t) };
        i.kh = function(n, t) { n.style.clip = yi(t) };
        i.Tb = function() { return +new Date };
        i.O = function(n, t) { n.appendChild(t) };
        i.ph = function(n, t) { c(t, function(t) { i.O(n, t) }) };
        i.pb = function(n, t, i) {
            (i || t.parentNode).insertBefore(n, t)
        };
        i.oh = function(n, t, i) { n.insertAdjacentHTML(t, i) };
        i.ib = function(n, t) {
            t = t || n.parentNode;
            t && t.removeChild(n)
        };
        i.ch = function(n, t) { c(n, function(n) { i.ib(n, t) }) };
        i.Rb = function(n) { i.ch(i.jb(n, u), n) };
        i.$Destroy = ki;
        i.Uc = function(n, t) {
            var u = i.Lb(n);
        ...
        dt = { $Top: r, $Right: r, $Bottom: r, $Left: r, Eb: r, Gb: r };
        i.Ch = function(n) {
            var t = i.bc(),
                u, f, r;
            return at(t, { Me: "block", Pb: i.db(n), $Top: 0, $Left: 0, Eb: 0, Gb: 0 }), u = i.ve(n, dt), i.pb(t, n), i.O(t, n), f = i.ve(n, dt), r = {}, c(u, function(n, t) { n == f[t] && (r[t] = n) }), at(t, dt), at(t, r), at(n, { $Top: 0, $Left: 0 }), r
        };
        i.Fh = function(n, t) { return parseInt(n, t || 10) };
        i.od = k;
        i.Ae = function(n, i) { for (var r = t.body; i && n !== i && r !== i;) i = i.parentNode; return n === i };
        i.ab = ei;
        i.Vb = function(n, t) {
            function f(n, u) {
        ...
        i.Ce = function(n, t, i, r) { var u, e, f, s; for (r && (n = ei(n)), u = wi(n, t), a(u) || (u = o.rg(n, t)), e = a(u) - 1; e > -1; e--) f = u[e], s = ei(i), lt(s, lt(f)), o.th(s, f.style.cssText), o.pb(s, f), o.ib(f); return n };
        i.Lc = function(n) { return new kr(n) };
        i.xh = function(n, t) { return s.xb(n * n + t * t) };
        i.R = ut;
        l("backgroundColor");
        i.Fb = l("overflow");
        ...
        i.Jf = function(n, t) {
            if (t === e) { t = ut(n, "backgroundImage") || ""; var i = /\burl\s*\(\s*["']?([^"'\r\n,]+)["']?\s*\)/gi.exec(t) || []; return i[1] }
            ut(n, "backgroundImage", t ? "url('" + t + "')" : "")
        };
        i.If = ot = { $Opacity: i.cb, $Top: i.V, $Right: i.Vd, $Bottom: i.be, $Left: i.U, Eb: i.K, Gb: i.J, Pb: i.db, Me: i.nb, $ZIndex: i.N, o: i.cb, x: i.U, y: i.V, i: i.N, dO: l("stroke-dashoffset", 1), ls: l("letterSpacing", 16), lh: l("lineHeight", 1), so: l("startOffset", 4, 100, d) };
        i.ve = function(n, t) { var i = {}; return c(t, function(t, r) { ot[r] && (i[r] = ot[r](n)) }), i };
        i.rb = at;
        nr = { cc: 0, Wb: 0, sX: 1, sY: 1, r: 0, rX: 0, rY: 0, tX: 0, tY: 0, tZ: 0, kX: 0, kY: 0 };
        ...
        i.Vf = function(n, t, i, r) {
            ...
            function y(n) {
                ...
                function i(n) {
                    var r = n & 1,
                        ...
                        t = 0,
                        i;
                    return n & 2 && (t = f % 2), i = n == 68 ? c : o.Zd(f, n), { Sd: n, ed: t, $Reverse: r, Od: i, Le: function(n) { return s.G((i[n] + t * !r) / u) } }
                }

                function l(n, t, i, r) {
                    var f = h(n, t, i),
                        u = 1 / (r * (f - 1) + 1);
                    return { nc: function(i) { return n.Od[i] < t && [n.Le(i) * r * u, u] } }
                }

                function a(n) { return { Pe: function(t) { return n.Od[t] % 2 ? 1 : -1 } } }

                function v(n, t, i, r, u) {
                    function c(n) { return s.B(1 - n / e, r) }
                    var e = h(n, t, i),
                        o = 0;
                    return n.Sd & 2 && (e = s.H(f / 2) - n.ed, o = c(e - 1) / 2 * !n.ed), { Se: function(t) { return t = n.Le(t), u && (t = e - t - 1), c(t) - o } }
                }
                var r = this,
                    c = o.Zd(f, 4),
                    t = o.Pd(n, ["b", "d", "e", "p", "dr"]) || {},
                ...
                r.Rf = function(n) {
                    ...
                    if (!h) {
                        ...
                        h = u[n] = { nc: nt.nc, Qf: function(n) { return (c != e ? it.Pe(n) : 1) * (w ? ut.Se(n) : 1) } }
                    }
                    return h
                }
            }
            ...
            o.cb(n, 1);
            c = { o: { j: 0, l: 1 } };
            h = { x: 0, y: 0, o: 1, r: 0, rX: 0, rY: 0, sX: 1, sY: 1, tZ: 0, kX: 0, kY: 0 };
            var a = l(n),
                f = o.A(a),
                v = [];
            o.f(a, function(n, u) {
    ...
    l = function(t, i, r, h, c, l) {
        ...
        function at(n, t, i) { w || (w = u, g = i, ht = t, n = s.l(n, y), n = s.j(n, v), b = n, k = b < p ? -1 : 1, a.Qc(), st = o.Tb(), rt(bt)) }

        function kt(n) { w && (g = w = ht = f, a.Rc(), n && n()) }
        var rt;
        t = t || 0;
        var a = this,
            d, w, b, k, g, st = 0,
        ...
        (o.He() && o.Cd() < 7 || !rt) && (rt = function(n) { o.$Delay(n, r.$Interval) });
        a.$Play = function(n, t, i) { at(n ? p + n : v, t, i) };
        a.tc = at;
        a.Ff = function(n, t) { at(v, n, t) };
        a.S = kt;
        a.Be = function() { return p };
        a.Ee = function() { return b };
        a.z = function() { return nt };
        a.L = ct;
        a.uf = function() { ct(v, u) };
        a.$IsPlaying = function() { return w };
        a.De = function(n) { vt = n };
        a.$Shift = dt;
        a.je = lt;
        a.M = function(n, t) { lt(n, 0, t) };
        a.zd = function(n) { lt(n, 1) };
        a.qd = function(n) { v += n };
        a.yc = function() { return y };
        a.vb = function() { return v };
        a.zc = a.Qc = a.Rc = a.kd = o.Kc;
        o.Tb();
        a.nf = function() { return d && d.E(1) };
        r = o.v({ $Interval: 16 }, r);
    ...
    d = n.$JssorSlideshowFormations$ = new function() {
        function ut(n) { return (n & b) == b }

        function ft(n) { return (n & k) == k }
    ...
    n.$JssorSlideshowRunner$ = function(n, t, i, e, c, a) {
        function p(n) {
            var t = { $Left: "x", $Top: "y", $Bottom: "m", $Right: "t", $Rotate: "r", $ScaleX: "sX", $ScaleY: "sY", $TranslateX: "tX", $TranslateY: "tY", $SkewX: "kX", $SkewY: "kY", $Opacity: "o", $ZIndex: "i", $Clip: "c" };
            o.f(n, function(i, r) {
                var u = t[r];
        ...
        function k(n, t) {
            var i = { $Duration: 1, $Delay: 0, $Cols: 1, $Rows: 1, $Opacity: 0, $Zoom: 0, $Clip: 0, $Move: f, $SlideOut: f, $Reverse: f, $Formation: d.$FormationRandom, $Assembly: 1032, $ChessMode: { $Column: 0, $Row: 0 }, $Easing: h.$Linear, $Round: {}, Ic: [], $During: {} };
            return o.v(i, n), i.$Rows == 0 && (i.$Rows = s.$Round(i.$Cols * t)), i.$Easing = o.Ld(i.$Easing), p(i), p(i.$Easing), p(i.$During), p(i.$Round), i.mc = i.$Cols * i.$Rows, i.Af = function(n, t) {
                ...
                if (n /= i.$Cols, t /= i.$Rows, f = n + "x" + t, !i.Ic[f])
                    for (i.Ic[f] = { w: n, h: t }, r = 0; r < i.$Cols; r++)
                        for (u = 0; u < i.$Rows; u++) i.Ic[f][u + "," + r] = { y: u * t, t: r * n + n, m: u * t + t, x: r * n };
                return i.Ic[f]
        ...
        function g(n, t, i, r, e, h) {
            var y, c, w = i.$ChessMode.$Column || 0,
                ...
                g = i.$SlideOut,
                v, k;
            return t = o.ab(t), o.N(t, 1), o.Fb(t, "hidden"), o.db(t, "absolute"), a(t, 0, 0), i.$Reverse || d.reverse(), v = { x: i.c & 1, t: i.c & 2, y: i.c & 4, m: i.c & 8 }, k = new l(0, 0), o.f(d, function(a, nt) {
                g && (nt = o.A(d) - nt - 1);
                var tt = i.$Delay * nt;
                o.f(a, function(a) {
                    var lt = a[0],
                    ...
                    yt && (et = p[ii], vt ? (d = {}, rt = i.$ScaleClip || 1, ot && st ? (d.y = p.h / 2 * rt, d.m = -d.y) : ot ? d.m = -p.h * rt : st && (d.y = p.h * rt), ht && ct ? (d.x = p.w / 2 * rt, d.t = -d.x) : ht ? d.t = -p.w * rt : ct && (d.x = p.w * rt), i.$Move && (bt = (d.x || 0) + (d.t || 0), kt = (d.y || 0) + (d.m || 0), c.x += bt, c.y += kt), y.c = et, o.f(d, function(n, t) { d[t] = et[t] + n }), c.c = d) : o.kh(ft, et));
                    dt = nt ? 1 : -1;
                    gt = it ? 1 : -1;
                    i.x && (c.x += e * i.x * dt);
                    i.y && (c.y += h * i.y * gt);
                    ni = { $Elmt: ft, $Easing: i.$Easing, $During: i.$During, $Round: i.$Round, $Move: i.$Move, Eb: e, Gb: h, le: s.$Round(i.$Duration / 4), $Reverse: !g };
                    c = o.Zg(c, y);
        ...
        v.of = function() { var n = r; return w && (n = new l(0, 0), n.M(w), n.M(b), n.ye = y), n }
    };
    var v = { qf: "data-scale", yb: "data-autocenter", Dd: "data-nofreeze", ze: "data-nodrag" },
        w = new function() {
            var n = this;
    ...
    n.$JssorBulletNavigator$ = function() {
        function et(n) { h[n] && h[n].Ye(n == l) }

        function ot(t) { n.k(k.uc, t * a) }
        var n = this,
            st = o.W(n, y),
            i, b, tt, it, d, l = 0,
        ...
        n.F = function(r, u) {
            ...
            n.gd = t = o.v({ $SpacingX: 10, $SpacingY: 10, $ActionMode: 1 }, u);
        ...
        n.$Destroy = function() { o.$Destroy(h, st) };
        o.F(n)
    };
    n.$JssorArrowNavigator$ = function() {
        function b(t) { n.k(k.uc, t, u) }
        ...
        n.$Destroy = function() { o.$Destroy(l, a, d) };
        o.F(n)
    };
    n.$JssorThumbnailNavigator$ = function() {
        ...
        h.nd = function(r, h) {
            ...
            pt = o.v({ $AutoPlay: 0, $NaviQuitDrag: f, $SlideWidth: tt, $SlideHeight: st, $SlideSpacing: a * v + p * (1 - v), $MinDragOffsetToSlide: 12, $SlideDuration: 200, $PauseOnHover: 1, $Cols: rt, $PlayOrientation: n.$Orientation, $DragOrientation: n.$NoDrag || n.$DisableDrag ? 0 : n.$Orientation }, n);
            b = new c(i, pt);
            w.Cc(i)
        };
        h.F = function(r, f, e) {
            i = r;
            h.gd = n = o.v({ $SpacingX: 0, $SpacingY: 0, $Orientation: 1, $ActionMode: 1 }, f);
            ut = o.K(i);
            ft = o.J(i);
        ...
        h.$Destroy = function() { o.$Destroy(b, l, st) };
        o.F(h)
    };
    p.pc = 21;
    p.Zb = 24;
    g = n.$JssorCaptionSlideo$ = function() {
        function lt(n) {
            var t = ct[n],
                i;
            return t == r && (t = ct[n] = { ob: n, Zc: [], Te: [] }, i = 0, o.f(d, function(t, r) { return i = r, t.ob > n }) || i++, d.splice(i, 0, t)), t
        }
        ...
        function ti(n, t) {
            var b = o.K(n),
                k = o.J(n),
                a = o.Ib(n),
                h = { x: a ? 0 : o.U(n), y: a ? 0 : o.V(n), cc: 0, Wb: 0, o: o.cb(n), i: o.N(n) || 0, r: 0, rX: 0, rY: 0, sX: 1, sY: 1, tX: 0, tY: 0, tZ: 0, kX: 0, kY: 0, ls: 0, lh: 1, so: 0, c: { y: 0, t: b, m: k, x: 0 } },
                i, f, v, e, d;
            if (ut && (v = ut[o.q(n, "c")], v && (i = lt(v.r, 0), i.sg = v.e || 0)), e = o.g(n, "data-to"), e && a && (e = o.Ud(e), e = { x: o.od(e[0]), y: o.od(e[1]) }), d = { $Elmt: n, $OriginalWidth: b, $OriginalHeight: k, Nf: e, Lf: h.x, Of: h.y, Ib: a }, o.f(t, function(t) {
                    var a = o.v({ $Easing: o.Ld(t.e), $During: t.dr, Bd: t.p }, d),
                        e = o.v(u, {}, t),
                        r, c;
                    o.Pd(e, ["b", "d", "e", "p", "dr"]);
                ...
                var c = f,
                    ...
                    g = f.vb(),
                    nt = s.l(g, i.sg);
                i.ob < g && (i.ob > p ? (c = new l(p, i.ob - p), c.M(f, u)) : c = r, y = new l(i.ob, nt - p, { hc: nt - i.ob, pf: u }), y.M(f, u));
                c && i.Zc.push(c);
                y && i.Te.push(y)
            }
            return h
        ...
        function at(t, i) {
            o.f(t, function(t) {
                var f = o.q(t, "play"),
                    r, u;
                i && f ? (r = new g(t, v, { xe: f }), wt.push(r), n.a(r, p.pc, ci), n.a(r, p.Zb, li)) : (at(o.jb(t).concat(o.xc(o.Hc(t, "data-tchd"))), i + 1), u = o.xc(o.Hc(t, "data-tsep")), u.push(t), o.f(u, function(n) {
                    var t = ht[o.q(n, "t")];
                    t && ti(n, t)
                }))
            })
        ...
        function it(n) { return n & 2 || n & 4 && o.ud().td }

        function ui() { ot || (e & 8 && n.a(t, "keydown", gt), e & 32 && (n.a(t, "mousedown", rt), n.a(t, "touchstart", rt)), ot = u) }
        ...
        function a() {!st && (b || n.z()) && (st = u, n.S(), n.Be() > n.qc && n.L(n.qc), n.De(yt || 1), n.tc(0)) }
        ...
        function gt(n) { e & 8 && o.Je(n) == 27 && a() }

        function oi(n) { c && o.Fe(n) !== r && (c = f, e & 16 && o.$Delay(ei, 160)) }

        function rt(n) { e & 32 && !o.Ae(i, o.$EvtSrc(n)) && a() }

        function si(n) { c || (c = u, h & 1 && o.Ie(n, i) && kt()) }
        ...
        n.zc = function() {!c && tt && n.Be() > n.fd && a() };
        n.F = function(t, r, f) {
            i = t;
            v = r;
            h = f.xe;
    ...
    c = n.$JssorSlider$ = (n.module || {}).exports = function() {
        function iu() { return !ui && hi & 12 }

        function bf() { return vu || !ui && hi & 3 }

        function ru() { return !rt && !iu() && !ot.$IsPlaying() }

        function we() { return !bf() && ru() }

        function kf() { return ut || ni }

        function be() { return kf() & 2 ? gi : di }
        ...
        function ne(n, t) { var u, r, i; return !rt || ft & 1 || (u = n, n < dt && (u = dt, r = -1), n > ri && (u = ri, r = 1), r && (i = n - u, t ? (i = s.Xf(i) * 2 / s.u, i = s.B(i * r, 1.6)) : (i = s.B(i * r, .625), i = s.Oe(i * s.u / 2)), n = u + i * r)), n }

        function te(n) { return ne(n, u) }

        function ke(n) { return ne(n) }
        ...
        function ie(n) { return !(ft & 1) && n - dt < .0001 }

        function re(n) { return !(ft & 1) && ri - n < .0001 }

        function or(n) { return !(ft & 1) && (n - dt < .0001 || ri - n < .0001) }

        function bu(n, t, i) { er || o.f(si, function(r) { r.Sc(n, t, i) }) }
        ...
        function ee(n, t) {!w || t && ot.$IsPlaying() || (ot.S(), realPosition = te(n), fi.L(realPosition), fe()) }

        function sr(n) { w ? (n = uu(n), n = it(n), yi = f, _IsStandBy = f, rt = f, ee(n)) : bu(0, 0) }
        ...
        function no() { oi && (c.Ke &= ~rr, oi = 0) }

        function oe(n) { var t = o.bc(); return o.rb(t, ei), n && o.Fb(t, "hidden"), t }

        function it(n, t) { return t = t || w || 1, (n % t + t) % t }
        ...
        function ku() { o.f(si, function(n) { n.id(n.gd.$ChanceToShow <= ui) }) }

        function to(n) { ui || !o.Fe(n) && o.Ie(n, d) || (ui = 1, ku(), rt || (hi & 12 && le(), tt[g] && tt[g].Bc()), i.k(c.$EVT_MOUSE_LEAVE)) }
        ...
        function du(n, t) { hr(n, t, u) }

        function hr(n, t, i, r) { var c, o, l, h, v, y, p;!w || rt && !a.$NaviQuitDrag || iu() || isNaN(n) || (c = et.z(), o = n, i && (o = c + n, ft & 2 && (ie(c) && n < 0 && (o = ri), re(c) && n > 0 && (o = dt))), ft & 1 || (o = r ? it(o) : uu(o, .5)), i && !or(o) && (o = s.$Round(o)), l = (o - c) % w, o = c + l, t == e && (t = br), h = s.P(l), v = 0, h && (h < 1 && (h = s.B(h, .5)), h > 1 && (y = be(), p = (ni & 1 ? ar : vr) / y, h = s.j(h, p * 1.5)), v = t * h), er = u, ot.S(), er = f, ot.se(c, o, v)) }

        function ro(n, t, i) {
            var a = this,
                h = { $Top: 2, $Right: 1, $Bottom: 2, $Left: 1 },
                y = { $Top: "top", $Right: "right", $Bottom: "bottom", $Left: "left" },
                f, r, e, c, l = {};
            a.$Elmt = n;
            a.$ScaleSize = function(a, p, w) {
                var b, k = a,
                    d = p,
                    g;
                e || (e = o.Ch(n), f = n.parentNode, c = { $Scale: o.q(n, v.qf, 1), $AutoCenter: o.q(n, v.yb) }, o.f(y, function(t, i) { l[i] = o.q(n, "data-scale-" + t, 1) }), r = n, t && (r = o.ab(f, u), o.rb(r, { $Top: 0, $Left: 0 }), o.O(r, n), o.O(f, r)));
        ...
        function fo() {
            var n = this,
                t = tu.$Elmt;
            l.call(n, -1, 2, { $Easing: h.$Linear, Pc: { Pb: gf }, hc: w, $Reverse: pr }, t, { Pb: 1 }, { Pb: -1 });
            n.Wc = t
        }
        ...
        function ho(n, t) {
            function ri() {
                ...
                vu -= vt;
                vt = 0;
                ut = new bi.$Class(rt, bi, { $Idle: o.q(rt, "idle", wr), cg: !b });
                ut.$On(p.Zb, si)
            }
            ...
            function ai() { g == t && k && (k.S(), nt && nt.$Quit(), nt && nt.$Disable(), k.he()) }

            function yi() { g == t && k && k.S() }

            function pi(n) { vi || i.k(c.$EVT_CLICK, t, n) }

            function fi(n, t) {
                var c, h, i, f, s, l, a;
                if (t || (d = o.$FindChild(n, "bg"), rt = d && o.Lb(d)), !o.g(n, fu) && (t || !d)) {
                    if (c = o.q(n, "data-arr"), c != e) {
                        function t(t, i) { o.g(t, i, o.g(n, i)) }
                        h = bi && bi.$Transitions || {};
            ...
            l.call(h, -kt, kt + 1, { hc: ft & 1 ? w : e, $Reverse: pr });
            h.Qe = function(t) { ii != t && (ii = t, t && o.O(n, yt), t || o.ib(yt)) };
            h.Vb = function(n, f) {
                f = f || h;
            ...
            h.Gg = function() {
                ...
                if (we())
                    if (w == 1) h.Jd(), dt(t, t);
                    else { if (ht && (n = ht.Bf(w)), n) return ct = o.Tb(), i = t + li, u = tt[it(i)], u.Vb(o.Z(r, hi, i, u, n, ct), h);!ft && or(et.z()) && or(et.z() + li) || du(li) }
            };
            h.Bc = function() { dt(t, t, u) };
            h.Jd = function() {
                nt && nt.$Quit();
                nt && nt.$Disable();
                h.Wd();
            ...
            h.Tg = function() { o.Fc(n) };
            h.Wd = function() { o.mb(n) };
            h.kd = function(n, t) {
                var i = kt - t;
                gf(gt, i)
            };
            ...
            h.a(i, 28, yi);
            h.a(i, 24, ai);
            h.$Destroy = function() { o.$Destroy(oi, ut, k) }
        }
        ...
        function ae(n) { o.kb(o.$EvtSrc(n), e, v.ze) || o.$CancelEvent(n) }
        ...
        function ye(n, r) {
            ...
            nu = 0;
            of = sf;
            r ? (f = n.touches[0], dr = { x: f.clientX, y: f.clientY }) : dr = o.Tc(n);
            u = o.$EvtSrc(n);
            s = o.kb(u, "1", rf);
        ...
        function gu(n) {
            var t, i, c, l;
            if (n = o.Zf(n), n.type != "mousemove" ? o.A(n.touches) == 1 ? (i = n.touches[0], t = { x: i.clientX, y: i.clientY }) : wi() : t = o.Tc(n), t) {
                var r = t.x - dr.x,
                    f = t.y - dr.y,
                    e = s.P(r),
                    h = s.P(f);
        ...
        i.$SlidesCount = function() { return w };
        i.$CurrentIndex = function() { return g };
        i.$CurrentPosition = function() { return et.z() };
        i.$Idle = function(n) {
            if (n == e) return wr;
            wr = n
        };
        ...
        i.$IsDragging = function() { return rt };
        i.$IsSliding = function() { return yi };
        i.$IsMouseOver = function() { return !ui };
        i.$LastDragSucceeded = function() { return vi };
        i.$OriginalWidth = function() { return pt };
        i.$OriginalHeight = function() { return wt };
        ...
        i.$ScaleSize = function(n, t, i) {
            ...
            ff = n / pt;
            ef = t / wt;
            o.f(hf, function(n) { n.$ScaleSize(ff, ef, i) });
        ...
        i.$PlayTo = hr;
        i.$GoTo = sr;
        i.$Next = function() { du(1) };
        i.$Prev = function() { du(-1) };
        i.$Pause = function() { b = 0 };
        i.$Play = function() { i.$AutoPlay(b || 1) };
        i.$SetSlideshowTransitions = function(n) { a.$SlideshowOptions.$Transitions = n };
        i.Id = function(n) {
            if (n = it(n), ft & 1) {
                var i = vt / st,
                    t = it(et.z()),
        ...
        i.$RemoveSlides = function(n) {
            var t = g,
                i = [];
            o.f(n, function(n) { n < w && n >= 0 && (i.push(ct[n]), n < g && t--) });
            nf(r, r, i);
            t = s.j(t, w - 1);
            sr(t)
        };
        i.F = function(n, r) {
            ...
            pt = o.K(d);
            wt = o.J(d);
            a = o.v({ $FillMode: 0, $LazyLoading: 1, $ArrowKeyNavigation: 1, $StartIndex: 0, $AutoPlay: 0, $Loop: 1, $HWA: u, $NaviQuitDrag: u, $AutoPlaySteps: 1, $Idle: 3e3, $PauseOnHover: 1, $SlideDuration: 500, $SlideEasing: h.$OutQuad, $MinDragOffsetToSlide: 20, $DragRatio: 1, $SlideSpacing: 0, $UISearchMode: 1, $PlayOrientation: 1, $DragOrientation: 1 }, r);
            a.$HWA = a.$HWA && o.gg();
            ...
            bi = o.v({ $Class: p }, a.$CaptionSliderOptions);
            ...
            lt = a.$SlideWidth || ar;
            at = a.$SlideHeight || vr;
            ei = { Eb: lt, Gb: at, $Top: 0, $Left: 0, Me: "block", Pb: "absolute" };
            bt = a.$SlideSpacing;
            di = lt + bt;
            gi = at + bt;
            st = ni & 1 ? di : gi;
...
function(n) { typeof define == "function" && define.amd ? define(["jquery"], n) : typeof exports == "object" ? n(require("jquery")) : n(jQuery) }(function(n) {
    ...
    function u(n) { return t.raw ? n : decodeURIComponent(n) }

    function f(n) { return i(t.json ? JSON.stringify(n) : String(n)) }

    function e(n) { n.indexOf('"') === 0 && (n = n.slice(1, -1).replace(/\\"/g, '"').replace(/\\\\/g, "\\")); try { return n = decodeURIComponent(n.replace(o, " ")), t.json ? JSON.parse(n) : n } catch (i) {} }

    function r(i, r) { var u = t.raw ? i : e(i); return n.isFunction(r) ? r(u) : u }
    var o = /\+/g,
...
! function(n) { "object" == typeof module && "object" == typeof module.exports ? n(require("jquery"), window, document) : n(jQuery, window, document) }(function(n, t, i) {
    var r = [],
    ...
    n.modal = function(t, i) {
        ...
        if (r.push(this), t.is("a"))
            if (f = t.attr("href"), this.anchor = t, /^#/.test(f)) {
                ...
                this.open()
            } else this.$elm = n("<div>"), this.$body.append(this.$elm), e = function(n, t) { t.elm.remove() }, this.showSpinner(), t.trigger(n.modal.AJAX_SEND), n.get(f).done(function(i) {
                if (n.modal.isActive()) {
    ...
    n.modal.prototype = {
        ...
        open: function() {
            ...
            this.options.clickClose && this.$blocker.click(function(t) { t.target === this && n.modal.close() })
        },
        close: function() {
            r.pop();
            this.unblock();
        ...
        unblock: function(t) {!t && this.options.doFade ? this.$blocker.fadeOut(this.options.fadeDuration, this.unblock.bind(this, !0)) : (this.$blocker.children().appendTo(this.$body), this.$blocker.remove(), this.$blocker = null, f(), n.modal.isActive() || this.$body.css("overflow", "")) },
        show: function() {
        ...
        hide: function() {
            ...
            var t = this;
            this.options.doFade ? this.$elm.fadeOut(this.options.fadeDuration, function() { t.$elm.trigger(n.modal.AFTER_CLOSE, [t._ctx()]) }) : this.$elm.hide(0, function() { t.$elm.trigger(n.modal.AFTER_CLOSE, [t._ctx()]) });
    ...
    n.modal.close = function(t) { if (n.modal.isActive()) { t && t.preventDefault(); var i = u(); return i.close(), i.$elm } };
    n.modal.isActive = function() { return r.length > 0 };
    n.modal.getCurrent = u;
    n.modal.defaults = { closeExisting: !0, escapeClose: !0, clickClose: !0, closeText: "Close", closeClass: "", modalClass: "modal", blockerClass: "jquery-modal", spinnerHtml: '<div class="rect1"><\/div><div class="rect2"><\/div><div class="rect3"><\/div><div class="rect4"><\/div>', showSpinner: !0, showClose: !0, fadeDuration: null, fadeDelay: 1 };
    ...
    n.fn.modal = function(t) { return 1 === this.length && new n.modal(this, t), this };
...
"function" != typeof Object.create && (Object.create = function(n) {
    ...
    function(n, t) {
        ...
        var i = {
            ...
            position: function() { "string" == typeof this.options.position && -1 !== n.inArray(this.options.position, this._positionClasses) ? "bottom-center" === this.options.position ? this._container.css({ left: n(t).outerWidth() / 2 - this._container.outerWidth() / 2, bottom: 20 }) : "top-center" === this.options.position ? this._container.css({ left: n(t).outerWidth() / 2 - this._container.outerWidth() / 2, top: 20 }) : "mid-center" === this.options.position ? this._container.css({ left: n(t).outerWidth() / 2 - this._container.outerWidth() / 2, top: n(t).outerHeight() / 2 - this._container.outerHeight() / 2 }) : this._container.addClass(this.options.position) : "object" == typeof this.options.position ? this._container.css({ top: this.options.position.top ? this.options.position.top : "auto", bottom: this.options.position.bottom ? this.options.position.bottom : "auto", left: this.options.position.left ? this.options.position.left : "auto", right: this.options.position.right ? this.options.position.right : "auto" }) : this._container.addClass("bottom-left") },
            bindToast: function() {
                var n = this;
                this._toastEl.on("afterShown", function() { n.processLoader() });
                this._toastEl.find(".close-jq-toast-single").on("click", function(t) { t.preventDefault(); "fade" === n.options.showHideTransition ? (n._toastEl.trigger("beforeHide"), n._toastEl.fadeOut(function() { n._toastEl.trigger("afterHidden") })) : "slide" === n.options.showHideTransition ? (n._toastEl.trigger("beforeHide"), n._toastEl.slideUp(function() { n._toastEl.trigger("afterHidden") })) : (n._toastEl.trigger("beforeHide"), n._toastEl.hide(function() { n._toastEl.trigger("afterHidden") })) });
                "function" == typeof this.options.beforeShow && this._toastEl.on("beforeShow", function() { n.options.beforeShow(n._toastEl) });
                "function" == typeof this.options.afterShown && this._toastEl.on("afterShown", function() { n.options.afterShown(n._toastEl) });
                "function" == typeof this.options.beforeHide && this._toastEl.on("beforeHide", function() { n.options.beforeHide(n._toastEl) });
                "function" == typeof this.options.afterHidden && this._toastEl.on("afterHidden", function() { n.options.afterHidden(n._toastEl) });
                "function" == typeof this.options.onClick && this._toastEl.on("click", function() { n.options.onClick(n._toastEl) })
            },
            addToDom: function() {
                var t = n(".jq-toast-wrap"),
                    i;
                (0 === t.length ? (t = n("<div><\/div>", { "class": "jq-toast-wrap", role: "alert", "aria-live": "polite" }), n("body").append(t)) : this.options.stack && !isNaN(parseInt(this.options.stack, 10)) || t.empty(), t.find(".jq-toast-single:hidden").remove(), t.append(this._toastEl), this.options.stack && !isNaN(parseInt(this.options.stack), 10)) && (i = t.find(".jq-toast-single").length - this.options.stack, i > 0 && n(".jq-toast-wrap").find(".jq-toast-single").slice(0, i).remove());
                this._container = t
            },
            ...
            animate: function() {
                if (n = this, this._toastEl.hide(), this._toastEl.trigger("beforeShow"), "fade" === this.options.showHideTransition.toLowerCase() ? this._toastEl.fadeIn(function() { n._toastEl.trigger("afterShown") }) : "slide" === this.options.showHideTransition.toLowerCase() ? this._toastEl.slideDown(function() { n._toastEl.trigger("afterShown") }) : this._toastEl.show(function() { n._toastEl.trigger("afterShown") }), this.canAutoHide()) {
                    var n = this;
                    t.setTimeout(function() { "fade" === n.options.showHideTransition.toLowerCase() ? (n._toastEl.trigger("beforeHide"), n._toastEl.fadeOut(function() { n._toastEl.trigger("afterHidden") })) : "slide" === n.options.showHideTransition.toLowerCase() ? (n._toastEl.trigger("beforeHide"), n._toastEl.slideUp(function() { n._toastEl.trigger("afterHidden") })) : (n._toastEl.trigger("beforeHide"), n._toastEl.hide(function() { n._toastEl.trigger("afterHidden") })) }, this.options.hideAfter)
                }
            },
            reset: function(t) { "all" === t ? n(".jq-toast-wrap").remove() : this._toastEl.remove() },
            update: function(n) {
        ...
        n.toast = function(n) { var t = Object.create(i); return t.init(n, this), { reset: function(n) { t.reset(n) }, update: function(n) { t.update(n) }, close: function() { t.close() } } };
        n.toast.options = { text: "", heading: "", showHideTransition: "fade", allowToastClose: !0, hideAfter: 3e3, loader: !0, loaderBg: "#9EC600", stack: 5, position: "bottom-left", bgColor: !1, textColor: !1, textAlign: "left", icon: !1, beforeShow: function() {}, afterShown: function() {}, beforeHide: function() {}, afterHidden: function() {}, onClick: function() {} }
...
function(n, t, i) {
    ...
    i.escapeSelector || (i.escapeSelector = function(n) { return (n + "").replace(/([\0-\x1f\x7f]|^-?\d)|^-$|[^\x80-\uFFFF\w-]/g, function(n, t) { return t ? "\0" === n ? "�" : n.slice(0, -1) + "\\" + n.charCodeAt(n.length - 1).toString(16) + " " : "\\" + n }) });
    i(function() {
...
function(n) {
    ...
    function i(t, r) {
        ...
        u.classes = { selected: "autocomplete-selected", suggestion: "autocomplete-suggestion" };
        u.hint = null;
        u.hintValue = "";
        u.selection = null;
        u.initialize();
    ...
    function f(n, t, i) { return n.value.toLowerCase().indexOf(i) !== -1 }

    function e(t) { return typeof t == "string" ? n.parseJSON(t) : t }

    function o(n, t) { if (!t) return n.value; var i = "(" + u.escapeRegExChars(t) + ")"; return n.value.replace(new RegExp(i, "gi"), "<strong>$1<\/strong>").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/&lt;(\/?strong)&gt;/g, "<$1>") }

    function s(n, t) { return '<div class="autocomplete-group">' + t + "<\/div>" }
    var u = function() { return { escapeRegExChars: function(n) { return n.replace(/[|\\{}()[\]^$+*?.]/g, "\\$&") }, createNode: function(n) { var t = document.createElement("div"); return t.className = n, t.style.position = "absolute", t.style.display = "none", t } } }(),
        t = { ESC: 27, TAB: 9, RETURN: 13, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40 },
        r = n.noop;
    i.utils = u;
    n.Autocomplete = i;
    i.defaults = { ajaxSettings: {}, autoSelectFirst: !1, appendTo: "body", serviceUrl: null, lookup: null, onSelect: null, width: "auto", minChars: 1, maxHeight: 300, deferRequestBy: 50, params: {}, formatResult: o, formatGroup: s, delimiter: null, zIndex: 9999, type: "GET", noCache: !0, onSearchStart: r, onSearchComplete: r, onSearchError: r, preserveInput: !1, containerClass: "autocomplete-suggestions", tabDisabled: !1, dataType: "text", currentRequest: null, triggerSelectOnValidInput: !0, preventBadQueries: !0, lookupFilter: f, paramName: "query", transformResult: e, showNoSuggestionNotice: !1, noSuggestionNotice: "No results", orientation: "bottom", forceFixPosition: !1, showLink: null };
    i.prototype = {
        initialize: function() {
            var t = this,
            ...
            r.on("mouseover.autocomplete", f, function() { t.activate(n(this).data("index")) });
            r.on("mouseout.autocomplete", function() {
                t.selectedIndex = -1;
                r.children("." + e).removeClass(e)
            });
            r.on("click.autocomplete", f, function() { t.select(n(this).data("index")) });
            r.on("click.autocomplete", function() { clearTimeout(t.blurTimeoutId) });
            t.fixPositionCapture = function() { t.visible && t.fixPosition() };
            n(window).on("resize.autocomplete", t.fixPositionCapture);
            t.el.on("keydown.autocomplete", function(n) { t.onKeyPress(n) });
            t.el.on("keyup.autocomplete", function(n) { t.onKeyUp(n) });
            t.el.on("blur.autocomplete", function() { t.onBlur() });
            t.el.on("focus.autocomplete", function() { t.onFocus() });
            t.el.on("change.autocomplete", function(n) { t.onKeyUp(n) });
            t.el.on("input.autocomplete", function(n) { t.onKeyUp(n) })
        },
        onFocus: function() {
            var t = this;
        ...
        setOptions: function(t) {
            ...
            n(r.suggestionsContainer).css({ "max-height": i.maxHeight + "px", width: i.width + "px", "z-index": i.zIndex });
            this.options = i
        },
        clearCache: function() {
            this.cachedResponse = {};
        ...
        onKeyPress: function(n) {
            var i = this;
            if (!i.disabled && !i.visible && n.which === t.DOWN && i.currentValue) { i.suggest(); return }
            if (!i.disabled && i.visible) {
                switch (n.which) {
                    case t.ESC:
                        ...
                        break;
                    case t.RIGHT:
                        if (i.hint && i.options.onHint && i.isCursorAtEnd()) { i.selectHint(); break }
                        return;
                    case t.TAB:
                        if (i.hint && i.options.onHint) { i.selectHint(); return }
                        if (i.selectedIndex === -1) { i.hide(); return }
        ...
        onKeyUp: function(n) {
            ...
            if (!i.disabled) {
                ...
                i.currentValue !== i.el.val() && (i.findBestHint(), i.options.deferRequestBy > 0 ? i.onChangeTimeout = setTimeout(function() { i.onValueChange() }, i.options.deferRequestBy) : i.onValueChange())
            }
        },
        onValueChange: function() {
            if (this.ignoreValueChange) { this.ignoreValueChange = !1; return }
            var t = this,
                r = t.options,
                u = t.el.val(),
                i = t.getQuery(u);
            if (t.selection && t.currentValue !== i && (t.selection = null, (r.onInvalidateSelection || n.noop).call(t.element)), clearTimeout(t.onChangeTimeout), t.currentValue = u, t.selectedIndex = -1, r.triggerSelectOnValidInput && t.isExactMatch(i)) { t.select(0); return }
            i.length < r.minChars ? t.hide() : t.getSuggestions(i)
        },
        isExactMatch: function(n) { var t = this.suggestions; return t.length === 1 && t[0].value.toLowerCase() === n.toLowerCase() },
        getQuery: function(t) {
        ...
        getSuggestionsLocal: function(t) {
            var f = this,
                ...
                i;
            return i = { suggestions: n.grep(r.lookup, function(n) { return o(n, t, e) }) }, u && i.suggestions.length > u && (i.suggestions = i.suggestions.slice(0, u)), i
        },
        getSuggestions: function(t) {
            ...
            if (r.params[r.paramName] = t, r.onSearchStart.call(i.element, r.params) !== !1) {
                ...
                u && Array.isArray(u.suggestions) ? (i.suggestions = u.suggestions, i.suggest(), r.onSearchComplete.call(i.element, t, u.suggestions)) : i.isBadQuery(t) ? r.onSearchComplete.call(i.element, t, []) : (i.abortAjax(), s = { url: f, data: e, type: r.type, dataType: r.dataType }, n.extend(s, r.ajaxSettings), i.currentRequest = n.ajax(s).done(function(n) {
                    var u;
                    i.currentRequest = null;
                    u = r.transformResult(n, t);
                    i.processResponse(u, t, o);
                    r.onSearchComplete.call(i.element, t, u.suggestions)
                }).fail(function(n, u, f) { r.onSearchError.call(i.element, t, n, u, f) }))
            }
        },
        ...
        suggest: function() {
            if (!this.suggestions.length) { this.options.showNoSuggestionNotice ? this.noSuggestions() : this.hide(); return }
            var t = this,
                i = t.options,
                o = i.groupBy,
                l = i.formatResult,
                ...
                f = "",
                c = i.showLink,
                e, y = function(n) { var t = n.data[o]; return e === t ? "" : (e = t, i.formatGroup(n, e)) };
            if (i.triggerSelectOnValidInput && t.isExactMatch(u)) { t.select(0); return }
        ...
        findBestHint: function() {
            var t = this,
                ...
                r = null;
            i && (n.each(t.suggestions, function(n, t) { var u = t.value.toLowerCase().indexOf(i) === 0; return u && (r = t), !u }), t.signalHint(r))
        },
        ...
        verifySuggestionsFormat: function(t) { return t.length && typeof t[0] == "string" ? n.map(t, function(n) { return { value: n, data: null } }) : t },
        validateOrientation: function(t, i) { return t = n.trim(t || "").toLowerCase(), n.inArray(t, ["auto", "bottom", "top"]) === -1 && (t = i), t },
...
function(n) {
    ...
    n.fn.textWidth = function() {
        var t = n("<span>" + n(this).html() + "<\/span>"),
            i, r;
        return (t.css("font-size", n(this).css("font-size")).hide(), t.prependTo("body"), i = t.width(), t.remove(), i == 0) ? (r = 0, n(this).eq(0).children().each(function() { r += n(this).textWidth() }), r) : i
    };
    ...
    Array.isArray = function(n) { return Object.prototype.toString.call(n) === "[object Array]" };
    String.prototype.getCodePointLength = function() { return this.length - this.split(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g).length + 1 };
    String.fromCodePoint = function() { for (var n, t = Array.prototype.slice.call(arguments), i = t.length; i-- > 0;) n = t[i] - 65536, n >= 0 && t.splice(i, 1, 55296 + (n >> 10), 56320 + (n & 1023)); return String.fromCharCode.apply(null, t) };
    n.fn.rate = function(i) {
        if (i === undefined || typeof i == "object") return this.each(function() { n.data(this, "rate") || n.data(this, "rate", new t(this, i)) });
        if (typeof i == "string") {
            var u = arguments,
                r;
            return this.each(function() {
    ...
    t.prototype.build = function() {
        ...
        n(this.element).css({ "-webkit-touch-callout": "none", "-webkit-user-select": "none", "-khtml-user-select": "none", "-moz-user-select": "none", "-ms-user-select": "none", "user-select": "none" });
        this.settings.hasOwnProperty("update_input_field_name") && this.settings.update_input_field_name.val(this.value)
    };
    t.prototype.addLayer = function(t, i, r, u) { for (var f, o = "<div>", e = 0; e < this.settings.max_value; e++) Array.isArray(r) ? (this.settings.convert_to_utf8 && (r[e] = String.fromCodePoint(r[e])), o += "<span>" + r[e] + "<\/span>") : (this.settings.convert_to_utf8 && (r = String.fromCodePoint(r)), o += "<span>" + r + "<\/span>"); return o += "<\/div>", f = n(o).addClass("rate-" + t).appendTo(this.element), n(f).css({ width: i + "%", height: n(f).children().eq(0).textHeight(), overflow: "hidden", position: "absolute", top: 0, display: u ? "block" : "none", "white-space": "nowrap" }), n(this.element).css({ width: n(f).textWidth() + "px", height: n(f).height(), position: "relative", cursor: this.settings.cursor }), f };
    t.prototype.updateServer = function() { this.settings.url != undefined && n.ajax({ url: this.settings.url, type: this.settings.ajax_method, data: n.extend({}, { value: this.getValue() }, this.settings.additional_data), success: n.proxy(function(t) { n(this.element).trigger("updateSuccess", [t]) }, this), error: n.proxy(function(t, i, r) { n(this.element).trigger("updateError", [t, i, r]) }, this) }) };
    t.prototype.getValue = function() { return this.value };
    ...
    t.prototype.toWidth = function(n) { return n / this.settings.max_value * 100 };
    ...
    t.prototype.getElement = function(t, i) { return n(this.element).find(".rate-" + t + " span").eq(i - 1) };
    t.prototype.getLayers = function() { return this.layers };
    t.prototype.setFace = function(n, t) { this.set_faces[n] = t };
    t.prototype.setAdditionalData = function(n) { this.settings.additional_data = n };
    t.prototype.getAdditionalData = function() { return this.settings.additional_data };
    t.prototype.removeFace = function(n) { delete this.set_faces[n] };
    t.prototype.setValue = function(t) {
        var u, i, r, e;
        if (!this.settings.readonly) {
            if (t < 0 ? t = 0 : t > this.settings.max_value && (t = this.settings.max_value), u = this.getValue(), this.value = t, e = n(this.element).trigger("change", { from: u, to: this.value }), n(this.element).find(".rate-face").remove(), n(this.element).find("span").css({ visibility: "visible" }), i = Math.ceil(this.value), this.set_faces.hasOwnProperty(i)) {
                ...
                n(o).appendTo(this.element).css({ display: "inline-block", position: "absolute", left: c }).addClass("rate-face");
                f.css({ visibility: "hidden" });
                s.css({ visibility: "hidden" });
                h.css({ visibility: "hidden" })
            }
            this.settings.only_select_one_symbol ? (r = this.toWidth(this.settings.max_value), this.layers.select_layer.css({ display: "block", width: r + "%", height: this.layers.base_layer.css("height") }), this.layers.hover_layer.css({ display: "none", height: this.layers.base_layer.css("height") }), this.layers.select_layer.children("span").css({ visibility: "hidden" }), this.layers.select_layer.children("span").eq(i != 0 ? i - 1 : 0).css({ visibility: "visible" })) : (r = this.toWidth(this.value), this.layers.select_layer.css({ display: "block", width: r + "%", height: this.layers.base_layer.css("height") }), this.layers.hover_layer.css({ display: "none", height: this.layers.base_layer.css("height") }));
            n(this.element).attr("data-rate-value", this.value);
            this.settings.change_once && (this.settings.readonly = !0);
            this.updateServer();
            e = n(this.element).trigger("afterChange", { from: u, to: this.value });
            this.settings.hasOwnProperty("update_input_field_name") && this.settings.update_input_field_name.val(this.value)
        }
    };
    t.prototype.increment = function() { this.setValue(this.getValue() + this.settings.step_size) };
    t.prototype.decrement = function() { this.setValue(this.getValue() - this.settings.step_size) };
    n.fn.rate.settings = { max_value: 5, step_size: .5, initial_value: 0, symbols: { utf8_star: { base: "☆", hover: "★", selected: "★" }, utf8_hexagon: { base: "⬡", hover: "⬢", selected: "⬢" }, hearts: "&hearts;", hhicon: '<i class="icon-star"><\/i>', fontawesome_beer: '<i class="fa fa-beer"><\/i>', fontawesome_star: { base: '<i class="fa fa-star-o"><\/i>', hover: '<i class="fa fa-star"><\/i>', selected: '<i class="fa fa-star"><\/i>' }, utf8_emoticons: { base: [128549, 128531, 128530, 128516], hover: [128549, 128531, 128530, 128516], selected: [128549, 128531, 128530, 128516] } }, selected_symbol_type: "utf8_star", convert_to_utf8: !1, cursor: "default", readonly: !1, change_once: !1, only_select_one_symbol: !1, ajax_method: "POST", additional_data: {} }
}(jQuery, window),
function(n, t) { typeof define == "function" && define.amd ? define(t) : typeof exports == "object" ? module.exports = t() : n.NProgress = t() }(this, function() {
    function f(n, t, i) { return n < t ? t : n > i ? i : n }

    function r(n) { return (-1 + n) * 100 }

    function l(n, i, u) { var f; return f = t.positionUsing === "translate3d" ? { transform: "translate3d(" + r(n) + "%,0,0)" } : t.positionUsing === "translate" ? { transform: "translate(" + r(n) + "%,0)" } : { "margin-left": r(n) + "%" }, f.transition = "all " + i + "ms " + u, f }

    function e(n, t) { var i = typeof n == "string" ? n : u(n); return i.indexOf(" " + t + " ") >= 0 }
    ...
    function u(n) { return (" " + (n && n.className || "") + " ").replace(/\s+/gi, " ") }

    function h(n) { n && n.parentNode && n.parentNode.removeChild(n) }
    var n = {},
        t, c, i;
    return n.version = "0.2.0", t = n.settings = { minimum: .08, easing: "linear", positionUsing: "", speed: 200, trickle: !0, trickleSpeed: 200, showSpinner: !0, barSelector: '[role="bar"]', spinnerSelector: '[role="spinner"]', parent: "body", template: '<div class="bar" role="bar"><div class="peg"><\/div><\/div><div class="spinner" role="spinner"><div class="spinner-icon"><\/div><\/div>' }, n.configure = function(n) { var i, r; for (i in n) r = n[i], r !== undefined && n.hasOwnProperty(i) && (t[i] = r); return this }, n.status = null, n.set = function(r) {
            var o = n.isStarted();
            r = f(r, t.minimum, 1);
            n.status = r === 1 ? null : r;
            var u = n.render(!o),
            ...
            return u.offsetWidth, c(function(f) {
                ...
                r === 1 ? (i(u, { transition: "none", opacity: 1 }), u.offsetWidth, setTimeout(function() {
                    i(u, { transition: "all " + e + "ms linear", opacity: 0 });
                    setTimeout(function() {
                        n.remove();
                        f()
                    }, e)
        ...
        }, n.isStarted = function() { return typeof n.status == "number" }, n.start = function() { n.status || n.set(0); var i = function() { setTimeout(function() { n.status && (n.trickle(), i()) }, t.trickleSpeed) }; return t.trickle && i(), this }, n.done = function(t) { return !t && !n.status ? this : n.inc(.3 + .5 * Math.random()).set(1) }, n.inc = function(t) { var i = n.status; return i ? i > 1 ? void 0 : (typeof t != "number" && (t = i >= 0 && i < .2 ? .1 : i >= .2 && i < .5 ? .04 : i >= .5 && i < .8 ? .02 : i >= .8 && i < .99 ? .005 : 0), i = f(i + t, 0, .994), n.set(i)) : n.start() }, n.trickle = function() { return n.inc() },
        function() {
            var i = 0,
                t = 0;
            ...
            var c = f.querySelector(t.barSelector),
                ...
                s;
            return i(c, { transition: "all 0 linear", transform: "translate3d(" + l + "%,0,0)" }), t.showSpinner || (s = f.querySelector(t.spinnerSelector), s && h(s)), e != document.body && o(e, "nprogress-custom-parent"), e.appendChild(f), f
        }, n.remove = function() {
            ...
            function r(n) { return n.replace(/^-ms-/, "ms-").replace(/-([\da-z])/gi, function(n, t) { return t.toUpperCase() }) }
            ...
            function f(n) { return n = r(n), i[n] || (i[n] = u(n)) }
...
$(document).ready(function() {
    $(".equaHeight").each(function(n, t) {
        var i = $(this).attr("data-obj"),
            r;
        i != "" && i != undefined ? (r = Math.max.apply(null, $(t).find(i).map(function() { return $(this).height() }).get()), $(t).find(i).height(r)) : $(t).children().ready(function() {
            setTimeout(function() {
                var n = Math.max.apply(null, $(t).children().map(function() { return $(this).height() }).get());
                $(t).children().height(n)
            }, 450)
        })
    });
    $("a#top").click(function() { $("html, body").animate({ scrollTop: 0 }, 500) });
...
setProductContentHeighWithSpecs = function() {
    ...
    $("#viewMoreContent").click(function() {
        ...
        $(this).text(n);
        n == "Thu gọn" ? $("#productContent").css({ height: "auto", overflow: "auto" }) : $("#productContent").css({ height: specsHeight, overflow: "hidden" })
    })
};
$.fn.serializeObject = function() {
    var n = {},
        t = this.serializeArray();
    return $.each(t, function() { n[this.name] ? (n[this.name].push || (n[this.name] = [n[this.name]]), n[this.name].push(this.value || "")) : n[this.name] = this.value || "" }), n
};
jQuery.fn.extend({
    autoHeight: function() {
        function n(n) { return jQuery(n).css({ height: "auto", "overflow-y": "hidden" }).height(n.scrollHeight - 40) }
        return this.each(function() { n(this).on("input", function() { n(this) }) })
    }
});
$(function() {
    var i = jQuery.ajax,
        n, t;
    jQuery.ajaxSetup({ timeout: 6e4 });
    jQuery.ajax = function() {
        return NProgress.start(), i.apply(this, arguments).done(function() { NProgress.done() }).fail(function() {
            $.toast({ text: "Có lỗi rảy ra, vui lòng bấm F5 để thử lại" });
            NProgress.done()
        })
    };
    n = jQuery.get;
    jQuery.get = function() {
        return NProgress.start(), n.apply(this, arguments).done(function() { NProgress.done() }).fail(function() {
            $.toast({ text: "Có lỗi rảy ra, vui lòng bấm F5 để thử lại" });
            NProgress.done()
        })
    };
    ...
    jQuery.post = function() {
        return NProgress.start(), t.apply(this, arguments).done(function() { NProgress.done() }).fail(function() {
            $.toast({ text: "Có lỗi rảy ra, vui lòng bấm F5 để thử lại" });
            NProgress.done()
        })
    }
});
jQuery.event.special.touchstart = { setup: function(n, t, i) { t.includes("noPreventDefault") ? this.addEventListener("touchstart", i, { passive: !1 }) : this.addEventListener("touchstart", i, { passive: !0 }) } };
inStock = !1;
$(document).ready(function() {
    $("#kwd").autocomplete({ serviceUrl: "/api/search", deferRequestBy: 200, maxHeight: 450, formatResult: autoComplateFormat, showLink: "/tim-kiem?kwd=" });
    $('a[href^="#"]').on("click", function(n) {
        var t = $(this.getAttribute("href"));
        t.length && (n.preventDefault(), $("html, body").stop().animate({ scrollTop: t.offset().top - 10 }, 1e3))
    });
    $("#login-modal").click(function(n) {
        n.preventDefault();
        this.blur();
        $.get(this.href, function(n) { $(n).appendTo("body").modal({ escapeClose: !1, clickClose: !1, closeClass: "icon-minutes", closeText: " " }) })
    });
    $(".ajax-modal").click(function(n) {
        n.preventDefault();
        this.blur();
        $("#popup-modal").css({ width: "" });
        $("#popup-modal").css({ padding: "" });
        var t = $(this).attr("data-width") != null ? $(this).attr("data-width") : "",
            i = $(this).attr("data-padding") != null ? $(this).attr("data-padding") : "";
        $.get(this.href, function(n) { $("#popup-modal").html(n).css({ width: t }).css({ padding: i }).modal({ closeClass: "icon-minutes", closeText: " " }) })
    });
    $("header nav").scrollToFixed({ preFixed: function() { $(this).css("color", "blue") }, postFixed: function() { $(this).css("color", "") } });
    $("textarea").autoHeight();
...
sliderContent_init = function() {
    var i = $("#sliderContent").length,
        t, n;
    if (i) {
        t = { $AutoPlay: !0, $Idle: 6e5, $DragOrientation: 3, $ArrowNavigatorOptions: { $Class: $JssorArrowNavigator$, $ChanceToShow: 2, $AutoCenter: 0, $Steps: 1 }, $BulletNavigatorOptions: { $Class: $JssorBulletNavigator$ } };
...
imagePreview_init = function() {
    ...
    var r = { $AutoPlay: !0, $SlideDuration: 500, $Idle: 5e3, $ArrowNavigatorOptions: { $Class: $JssorArrowNavigator$ }, $PauseOnHover: 1, $ThumbnailNavigatorOptions: { $Class: $JssorThumbnailNavigator$, $SpacingX: 10, $SpacingY: 5, $ArrowNavigatorOptions: { $Class: $JssorArrowNavigator$ } } },
        n = new $JssorSlider$("imagePreview", r),
        i = $("#colorOptions div.selected").attr("data-idx");
    i && n.$GoTo(i);
    $("#versionOption").click(function() { $(this).find("a").click() });
    $(".slider-t-l").click(function() { n.$Prev() });
    $(".slider-t-r").click(function() { n.$Next() });
    $("#colorOptions > div").click(function() {
        ...
        var e = $(this).attr("data-bestPrice"),
            ...
            i = "<strong>" + e + "<\/strong>";
        u != "" && (i += "<i>Giá Niêm Yết: <strike>" + u + "<\/strike><\/i>");
        f != "" && (i += "<i>" + f + "");
